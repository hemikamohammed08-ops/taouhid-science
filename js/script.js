...
// ===== next script block ===== 
    ...
    function revealStageArea(scroll) {
        ...
        for (var i = 0; i < nodes.length; i++) { nodes[i].removeAttribute('data-stage-gate'); opened = true; }
        var target = document.getElementById('mainStagesContainer');
        if (target && scroll !== false) {
            setTimeout(function(){ target.scrollIntoView({behavior:'smooth', block:'start'}); }, opened ? 80 : 0);
        }
    }
